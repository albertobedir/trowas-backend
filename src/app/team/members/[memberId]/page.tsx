...
import { MdCall, MdSave } from "react-icons/md";
...
import { LinksCategoryModal } from "@/components/ui/LinksCategoryModal";
...
const MemberDetailPage = () => {
  ...
  useEffect(() => {
    if (data?.data?.userCard?.emailData) {
      setEmailSubject(data?.data?.userCard?.emailData?.subject);
      setEmailBody(data?.data?.userCard?.emailData?.message);
      setEmailAfterHour(data?.data?.userCard?.emailData?.sendAfterHour);
      setEmailAfterMinute(data?.data?.userCard?.emailData?.sendAfterMinute);
      setAutofollowisactive(data?.data?.userCard?.emailData?.isActive);
    }
  ...
  const member = data?.data?.userCard || defaultMember;

  const linkData = data?.data?.userCard.links.links;
  ...
  // Generate QR code as SVG for overlay
  useEffect(() => {
  ...
  const defaultFormHeader = `Share your info back with card owner`;
  ...
  // Helper function to parse template variables - keep raw for highlighting in preview
  const parseTemplateVariables = (text: string, memberData: any) => {
    // For preview, we keep the template variables as-is for highlighting
    return text;
  };
  ...
  const handleLeadCaptureFormSave = () => {
    try {
      ...
      Api.patch(`/user/${memberId}/card/form?index=${index}`, jsonData);
      ...
      // For example:
      // Api.patch(`/user/${memberId}/card/leadCapture`, jsonData)
  ...
  const leadData = data?.data.userCard.leadForm;
  ...
  // Handle virtual background upload
  const handleVirtualBackgroundUpload = () => {
    virtualBackgroundInputRef.current?.click();
  };
  ...
  const {
    ...
    defaultValues: {
      ...
      company: data?.data?.userCard?.company || "",
      call: data?.data?.userCard?.call || "",
      email: data?.data?.userCard?.email || "",
      bio: member?.bio || "",
    },
  }); // Update form values when API data is loaded - with proper null/undefined handling
  React.useEffect(() => {
    if (data?.data?.userCard) {
      reset({
        name: data.data.userCard?.name || "",
        cardName: data.data.userCard?.cardName || "",
        cardLayout: data.data.userCard?.cardLayout || "Left Aligned",
        location: data.data.userCard?.location || "",
        jobTitle: data.data.userCard?.jobTitle || "",
        company: data.data.userCard?.company || "",
        call: data.data.userCard?.call || "",
        email: data.data.userCard?.email || "",
        bio: data.data.userCard?.bio || "",
      });
      setProfileImage(data.data.userCard?.profilePicture || "/defaultpp.png");
      setCoverImage(data.data.userCard?.coverPhoto || "/defaultpp.png");
      setCompanyLogo(data.data.userCard?.companyLogo || "/defaultpp.png");
      setSelectedCardTheme(data.data.userCard?.cardTheme || colorOptions[0]);
      setSelectedLinkColor(data.data.userCard?.linkColor || colorOptions[0]);

      // Load lead form data if available
      if (data.data.userCard.leadForm) {
        const leadForm = data.data.userCard.leadForm;
      ...
      // Legacy support for older data format
        ...
        data.data.userCard?.activeLinks ||
        data.data.userCard?.linkOrder
      ) {
        try {
          if (data.data.userCard?.activeLinks) {
            const savedLinks = JSON.parse(data.data.userCard.activeLinks);
            setActiveLinks({ ...activeLinks, ...savedLinks });
          }

          if (data.data.userCard?.linkOrder) {
            const savedLinkOrder = JSON.parse(data.data.userCard.linkOrder);
  ...
  }, [data, reset, colorOptions, defaultFormHeader, linkData]);
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: Mail,
      description: "Email templates for follow-ups",
    },
    {
      name: "QR Code",
      value: "qr-code",
    ...
    },
    {
      ...
      icon: ImageIcon,
      description: "Virtual meeting backgrounds",
    },
    {
  ...
  const renderContent = () => {
    switch (activeTab) {
            ...
            <div className="space-y-6 p-4  rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm  font-normal block">Name</label>{" "}
                  <Controller
                    name="name"
                    control={control}
                    render={({ field }) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-normal block">Job Title</label>{" "}
                  <Controller
                    name="jobTitle"
                    control={control}
                    render={({ field }) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-normal block">Call</label>{" "}
                  <Controller
                    name="call"
                    control={control}
                    render={({ field }) => (
                ...
                <div className="space-y-2">
                  <label className="text-sm font-normal block">Email</label>{" "}
                  <Controller
                    name="email"
                    control={control}
                    render={({ field }) => (
              ...
              <div className="space-y-2">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <textarea
                      className="w-full px-3 py-3 resize-none rounded-[10px] bg-[#F7F7F7] text-sm font-normal focus:outline-none min-h-[100px] read-only:bg-gray-100 read-only:text-gray-500"
                      placeholder="Enter bio"
                      {...field}
                      readOnly={hasPermission(BigInt(userperm), BitPerms.bio)}
                      onClick={() => {
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  checked={autofollowisactive}
                  onCheckedChange={setAutofollowisactive}
                />
              </div>
            </div>
              ...
              <div className="flex gap-4">
                <div className="w-full relative">
                  <input
                    type="number"
                    min="0"
                    value={emailAfterHour}
                    onChange={(e) => setEmailAfterHour(e.target.value)}
                ...
                <div className="w-full relative">
                  <input
                    type="number"
                    min="0"
                    value={emailAfterMinute}
                    onChange={(e) => setEmailAfterMinute(e.target.value)}
                    className="w-full px-3 py-3 pr-16 rounded-[10px] bg-[#F7F7F7] focus:outline-none"
                  />
                  <span className="absolute top-1/2 right-4 transform -translate-y-1/2 text-sm text-gray-500">
                    Minutes
      ...
      case "virtual-background":
        if (!hasPermission(BigInt(userperm), BitPerms.virtualbackground)) {
          return (
            <motion.div
              className="space-y-6"
              initial={{ opacity: 0, y: 20 }}
                ...
                >
                  <div className="flex justify-between items-center mb-3 bg-gray-50/70 p-3 rounded-lg">
                    ...
                    <motion.button
                      ...
                      onClick={handleVirtualBackgroundUpload}
                    >
                ...
                {linkOrder.map((linkKey: string, index) => {
                  ...
                  if (linkKey === "email") {
                    ...
                    iconComponent = (
                      <div className="relative w-6 h-6">
                        <Image
                          src={linkImages.instagram || "/links/instagram.svg"}
                          alt="Instagram"
                          width={24}
                          height={24}
                          sizes="24px"
                    ...
                    iconComponent = (
                      <div className="relative w-6 h-6">
                        <Image
                          src={linkImages.linkedin || "/links/linkedin.svg"}
                          alt="LinkedIn"
                          width={24}
                          height={24}
                          sizes="24px"
                    ...
                    iconComponent = (
                      <div className="relative w-6 h-6">
                        <Image
                          src={linkImages.website || "/links/safari.svg"}
                          alt="Website"
                          width={24}
                          height={24}
                          sizes="24px"
                    ...
                    iconComponent = (
                      <div className="relative w-6 h-6">
                        <Image
                          src={linkImages.contact || "/links/contactcard.svg"}
                          alt="Contact"
                          width={24}
                          height={24}
                          sizes="24px"
                    ...
                    iconComponent = (
                      <div className="relative w-6 h-6">
                        <Image
                          src={linkImages.discord || "/links/discord.svg"}
                          alt="Discord"
                          width={24}
                          height={24}
                          sizes="24px"
                    ...
                    iconComponent = (
                      <div className="relative w-6 h-6">
                        <Image
                          src={linkImages.facetime || "/links/facetime.svg"}
                          alt="FaceTime"
                          width={24}
                          height={24}
                          sizes="24px"
                    ...
                    iconComponent = (
                      <div className="relative w-6 h-6">
                        <Image
                          src={linkImages.calendly || "/links/calendly.svg"}
                          alt="Calendly"
                          width={24}
                          height={24}
                          sizes="24px"
                    ...
                    iconComponent = (
                      <div className="relative w-6 h-6">
                        <Image
                          src={linkImages.file || "/links/file.svg"}
                          alt="File"
                          width={24}
                          height={24}
                          sizes="24px"
      ...
      case "lead-capture-form":
        return (
          <LeadCaptureViewCard
            ...
            formDisclaimer={formDisclaimer}
            hiddenFields={hiddenFields}
            cardUrl={`/connect/${memberId}?index=${index}`}
          />
        );
      case "follow-up-email":
        return (
      ...
      case "follow-up-email":
        return (
          <EmailPreview
            ...
            emailSubject={parseTemplateVariables(emailSubject, member)}
            emailGreeting={parseTemplateVariables(emailGreeting, member)}
            emailBody={parseTemplateVariables(emailBody, member)}
          />
        );
      case "about":
        if (cardLayoutLiv === "Center Aligned") {
          return (
            <div className="flex flex-col items-center h-full">
              ...
              <div
                className="w-64 h-[520px] flex flex-col moder overflow-auto rounded-[25px] mt-4 hidden-scrollbar border border-[rgb(189,189,189)]"
                style={{
                  backgroundColor:
                    selectedCardTheme === "transparent"
                      ? "#BFDBFE"
                        ...
                        {linkOrder
                          ...
                          .map((linkKey) => {
                            ...
                            return (
                              <div
                                key={linkKey}
                                className="flex flex-row-reverse justify-between items-center w-full py-1 pl-2  rounded-md"
                                style={{
                                  backgroundColor:
                                    selectedLinkColor === "transparent"
                                      ? "#3B82F6"
          ...
          try {
            ...
            if (parentContainer) {
              ...
              if (originalTransformOrigin !== undefined) {
                parentContainer.style.transformOrigin = originalTransformOrigin;
              }
            }
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div className="text-sm text-blue-700">
                    ...
                    <ol className="list-decimal list-inside space-y-1 text-blue-600">
                      <li>Download the signature image</li>
                      <li>Open your email client settings</li>
                      <li>Upload the image as your signature</li>
      ...
      case "virtual-background":
                    ...
                    try {
                      ...
                      backgroundImg.onload = async () => {
                        ...
                        if (relativeContainer) {
                          ...
                          // No overlays, just download the background
        ...
        return (
          <motion.div
            ...
            className="space-y-4"
          >
            <h2 className="text-xl font-semibold">{`${member.name}'s QR Code`}</h2>
            <div className="space-y-4">
              <QrCodePreview
            ...
            <div className="flex items-center gap-2">
              ...
              <Popover>
                ...
                <PopoverContent className="w-80 p-5 rounded-2xl" align="end">
                  ...
                  {/* Current active card */}
                  <div className="p-4 hover:bg-slate-50 rounded-xl cursor-pointer relative">
                  ...
                  <div className="p-4 border-t">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium">Other Cards</span>
                    </div>
                  </div>
                    ...
                    <div className="mt-auto pt-44">
                      ...
                      <div className="px-2.5">
                        <div
                          ...
                          className="flex items-center justify-between rounded-full border p-2 bg-slate-50 cursor-pointer hover:bg-slate-100"
                        >
                          <div className="flex items-center gap-2">
