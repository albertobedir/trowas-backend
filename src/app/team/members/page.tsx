...
function MembersContent() {
  ...
  const [templateMode, setTemplateMode] = useState<"single" | "bulk">("single");
  ...
  const [subteamlistData, setSubteamlistData] = useState<
    Array<{
      _id: string;
      name: string;
      description: string;
  ...
  const [isBulkSubteamOpen, setIsBulkSubteamOpen] = useState(false);

  const downloadQr = async (memberId: string) => {
    try {
      const res = await Api.get(
        `/teams/${user?.team}/members/${memberId}/qr-code`,
        {
          responseType: "blob",
        },
      );
      ...
      link.href = url;
      link.setAttribute("download", `${memberId}-qr.svg`);
  ...
  useEffect(() => {
    if (!isUserLoading && user && isIndividualAccount(user)) {
  ...
  useEffect(() => {
    const fetchSubteamlistData = async () => {
      if (!isUserLoading && user?.team && !isIndividualAccount(user)) {
        try {
          const response = await Api.get(
            `/subteam/get-all?teamId=${user.team}`,
          );
          if (response.data?.subTeams) {
  ...
  const handleBulkAssignTemplate = async (templateId: string) => {
    await Api.post(`/teams/${user?.team}/members/bulk-assign-template`, {
  ...
  const handleBulkAssignSubteam = async (subteamId: string) => {
    await Api.post(`/teams/${user?.team}/members/bulk-assign-subteam`, {
      memberIds: selectedItems,
      subteamId,
    });

    toast.success("Subteam assigned");
    fetchMembers();
  };
  const handleImageUpload = async (
    type: "profilePicture" | "companyLogo" | "coverPhoto",
    memberId: string,
    selectedItems: string[],
      ...
      input.addEventListener("change", async (e) => {
        ...
        try {
          ...
          const response = await Api.post(
            `/teams/${teamId}/members/change-photos`,
            formData,
            {
              headers: {
  ...
  const menuItems: MenuItem[] = [
    {
      icon: (
        <Upload className="h-3.5 w-3.5 mr-2 transition-colors group-hover:text-blue-500" />
      ),
    ...
    },
    {
      ...
      variant: "default",
      onClick: (memberId) =>
        handleImageUpload("coverPhoto", memberId, selectedItems, user?.team),
      divider: true,
    },
    {
      icon: (
      ...
      variant: "default",
      onClick: (memberId) =>
        handleImageUpload("companyLogo", memberId, selectedItems, user?.team),
    },
  ];
  ...
  useEffect(() => {
    ...
    if (templates)
      urlFilters["Templates"] = templates.split(",").filter(Boolean);
  ...
  const applyFilters = (members: TeamMember[]) => {
    ...
    return members.filter((member) => {
      ...
      const statusMatch =
        !selectedFilters["Member Status"]?.length ||
        selectedFilters["Member Status"].some(
          (filter) =>
            filter.toLowerCase() === member.roles.teamRole.toLowerCase(),
        );

      const subteamsMatch =
        !selectedFilters["Subteams"]?.length ||
      ...
      const templateMatch =
        !selectedFilters["Templates"]?.length ||
        selectedFilters["Templates"].some(
          (filter) =>
            (member.template?.toLowerCase() || "no template") ===
            filter.toLowerCase(),
        );
  ...
  const filterGroups = [
    ...
    },
    {
      ...
      items: Array.from(
        new Set(
          members
            ?.map((member) => member.template || "No Template")
            .filter(Boolean),
        ),
      ),
    },
                          ...
                          >
                            ...
                            <div className="absolute bottom-0 left-0 right-0 flex justify-between p-4 border-t bg-background/80 backdrop-blur-sm">
                              ...
                              <Button
                                ...
                                variant={
                                  hasSelectedFilters ? "default" : "secondary"
                                }
                                onClick={() => {
                    ...
                    <Popover>
                      ...
                      <PopoverContent className="rounded-xl w-[320px] p-4">
                        <motion.div
                          ...
                          transition={springConfig}
                        >
                          <h4 className="text-sm font-medium mb-4 text-black/70">
                            Sort by
                          </h4>
                            ...
                            <TableRow className="h-[48px]">
                              <TableHead className="w-[300px]">
                                <div className="flex items-center gap-4">
                                  <Checkbox
                                    checked={
                                      selectedItems.length === members.length
                                    }
                                  ...
                                  <TableCell className="min-w-0">
                                    <div className="flex items-center gap-4 ml-4 min-w-0">
                                      <Checkbox
                                        ...
                                        className="shrink-0 data-[state=checked]:bg-black data-[state=checked]:border-black transition-all duration-200 group-hover:border-black"
                                      />
                                      <div
                                        onClick={() =>
                                          handlepermClick(item._id)
                                        }
                          ...
                          .map((item) => (
                            ...
                            >
                              <Card className="overflow-hidden group relative">
                                <div className="relative h-24 overflow-hidden bg-gradient-to-r from-blue-500 to-blue-600">
                                  {/* eslint-disable-next-line @next/next/no-img-element */}
                                  <img
                                    src={item.coverPhoto || "/defaultcover.jpg"}
                                    alt={`${item.name} cover`}
                                ...
                                <Link
                                  href={`/team/members/${item._id}?index=0`}
                                >
                                  <div className="flex flex-col items-center -mt-12 relative px-6">
                                    ...
                                    <div className="mt-3 w-full text-center">
                                      ...
                                      <p
                                        className="text-sm text-muted-foreground mt-1 truncate px-2"
                                        title={item.username}
                                      >
                                        {item.username}
                                      </p>
                                ...
                                <div className="flex items-center gap-2 mt-4 mb-6 justify-center px-4">
                                  <div className="shrink-0 rounded-full bg-[rgb(242,242,242)] px-4 py-1.5 text-xs font-semibold uppercase text-[rgb(79,79,79)]">
                                    {item.roles.teamRole}
                                  </div>
                                  <Button
                                    size="sm"
                                ...
                                <div className="absolute top-3 right-3">
                                  {/* <Popover>
                                    ...
                                    <PopoverContent
                                      className="w-48 py-7 rounded-lg px-2"
                                      align="end"
                                      alignOffset={-5}
                                      sideOffset={5}
                                    >
                        ...
                        <div className="flex-1 flex items-center justify-between gap-1">
                          <div className="flex items-center gap-[2px]">
                            <div className="flex items-center">
                              ...
                              <Separator
                                orientation="vertical"
                                className="h-6 bg-white/20 mx-0.5"
                              />
                                    ...
                                    <div className="flex flex-col gap-0.5">
                                      ...
                                      >
                                        <Button
                                          ...
                                          onClick={() => {
                                            ...
                                            for (const id of selectedItems) {
                                              setTimeout(() => {
                                                window.open(
                                                  `/api/teams/${user?.team}/members/${id}/qr-code`,
                                                  "_blank",
                                                );
                                              }, 200);
                                            }
                                        ...
                                        >
                                          ...
                                          <span className="transition-colors group-hover:text-blue-500 text-[13px]">
                                            Download QR Code
                                          </span>
                                        </Button>
                                      </motion.div>
                          ...
                          <Separator
                            orientation="vertical"
                            className="h-6 bg-white/20 mx-1"
                          />
                          ...
                          <div className="flex items-center">
                            ...
                            <motion.button
                              ...
                              onClick={() => {
                                if (!selectedItems.length)
                                  return toast.error("No members selected");

                                setTemplateMode("bulk");
                                setTemplateModalOpen(true);
                              }}
                            >
                            ...
                            <Popover>
                              <PopoverTrigger asChild>
                                <motion.button
                                  whileTap={{ scale: 0.97 }}
                                  className="h-9 px-3 text-sm font-medium text-white hover:bg-white/20 transition-all duration-200 flex items-center gap-2 rounded-md"
                                >
                                  <Users2 className="h-[15px] w-[15px]" />
                                  Assign to Subteam
                                ...
                                <div className="flex flex-col gap-1">
                                  {subteamlistData.map((subteam) => (
                                    ...
                                    >
                                      <button
                                        onClick={() => {
                                          handleBulkAssignSubteam(subteam._id);
                                          setSelectedItems([]); // bonus UX
                                        }}
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button
                onClick={async () => {
                  try {
                    const response = await Api.post(
                      `/teams/${user?.team}/members/change-passwords`,
                      {
                        memberIds: selectedItems,
                        newPassword,
                      },
