...
export function TemplateSelectionDialog({
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<Template | null>(null);  const handleSelectTemplate = (template: Template) => {
...
const {mutate} = useMutation({
    mutationFn: async (selectedTemplateId:string) => {
      ...
      const response = await Api.patch(`/user/${memberId}/card/update`, formData, {
        headers: {
        ...
        <div className="flex flex-col rounded-3xl h-[750px]">
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className="w-2/5 border-r overflow-y-auto p-4 custom-scrollbar">
              <div className="space-y-3">
                {!isLoading && templateList.map((template: Template) => (
                  <div
                    key={template._id}
            ...
            <div className="w-3/5 p-6 flex flex-col items-center justify-center bg-gray-50 custom-scrollbar">              {selectedTemplate ? (
                <LiveTemplatePreview
                  template={{
                    id: selectedTemplate._id,
                    name: selectedTemplate.templateName,
                    layout: selectedTemplate.cardLayout as 'Center Aligned' | 'Left Aligned' | 'Portrait',
                    cardTheme: selectedTemplate.cardTheme || 
                               (selectedTemplate.cardLayout === 'Center Aligned' ? '#FFFFFF' : 
                               selectedTemplate.cardLayout === 'Left Aligned' ? '#F8FAFC' :
                               selectedTemplate.cardLayout === 'Portrait' ? '#F3F4F6' : '#FFFFFF'),
                  ...
                  profileData={{
                    ...
                    bio: selectedTemplate.bio || "Passionate about creating innovative solutions that make a difference. Always learning and growing in the tech space."
                  }}
                  profileImage={selectedTemplate.profilePicture || "/defaultpp.png"}
                  coverImage={selectedTemplate.coverPhoto}
                  companyLogo={selectedTemplate.companyLogo || "/company_logo.png"}
                  activeLinks={selectedTemplate.links?.links?.activeLinks || {
                    linkedin: true,
                    twitter: true,
                    email: true,
                    website: true
                  }}
                  linkOrder={selectedTemplate.links?.links?.linkOrder || ['linkedin', 'twitter', 'email', 'website']}
                />
              ) : (
          ...
          <div className="border-t p-4 flex justify-end items-center">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleAssign} className="rounded-full">İmport to Card</Button>
            </div>
          </div>
        </div>
      </DialogContent>
